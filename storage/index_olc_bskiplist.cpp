...
RC IndexOLCBskiplist::init(int part_cnt) {
    _bsl_list = bsl_new();
...
RC IndexOLCBskiplist::init(int part_cnt, table_t * table) {
    this->table = table;
...
bool IndexOLCBskiplist::index_exist(idx_key_t key) {
    return bsl_get(_bsl_list, (bsl_key_t)key + 1, nullptr) == 1;
}

RC IndexOLCBskiplist::index_insert(idx_key_t key, itemid_t * item, int part_id, uint64_t thd_id) {
    item->next = nullptr;
    int res = bsl_insert(_bsl_list, (bsl_key_t)key + 1, (bsl_val_t)item);
    M_ASSERT(res == 1, "Failed to insert into olc-bskiplist!");
    return RCOK;
}

RC IndexOLCBskiplist::index_read(idx_key_t key, itemid_t * &item, int part_id, int thd_id) {
    bsl_val_t val = 0;
...
RC IndexOLCBskiplist::index_next(uint64_t thd_id, itemid_t * &item, bool samekey) {
    ...
    if (!samekey && !next_item) {
        OLCBskiplistScanCursor &sc = scan_cursor_per_thd[thd_id];

        if (sc.pos >= sc.count) {
            idx_key_t cur_key = cur_key_per_thd[thd_id * PREFETCH_SIZE_WORDS];
            BatchFillArg arg = { sc.keys, sc.vals, 0, OLC_BSKIPLIST_SCAN_BATCH };
            bsl_scan_n_batch(_bsl_list, (bsl_key_t)(cur_key + 1) + 1,
                              OLC_BSKIPLIST_SCAN_BATCH, bsl_scan_batch_callback, &arg);
            sc.count = arg.count;
            sc.pos = 0;
        }
