...
#define BTREEOLC_SCAN_BATCH 32

struct BTreeOLCScanCursor {
	uint64_t 	keys[BTREEOLC_SCAN_BATCH];
	uint64_t 	vals[BTREEOLC_SCAN_BATCH];
	int 		count;	// number of valid entries currently in the buffer
	int 		pos;	// index of the next unread entry
};

class IndexBTreeOLC : public index_base
{
public:
...
private:
	void * 			_btree;	// really a btreeolc::BTree<uint64_t, uint64_t>*, see index_BTreeOLC.cpp
	...
	BTreeOLCScanCursor scan_cursor_per_thd[THREAD_CNT];
};
