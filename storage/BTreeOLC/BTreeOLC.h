...
namespace btreeolc_dbx1000_detail {
...
struct BTreeLeaf : public BTreeLeafBase {
  ...
  void insert(Key k,Payload p) {
	...
	// Upsert -- chain the new item onto the old one (DBx1000 non-unique
	// index support) rather than dropping it.
	btreeolc_dbx1000_detail::dbx1000_chain_onto((uint64_t)payloads[pos], (uint64_t)p);
	payloads[pos] = p;
	return;
      }
...
struct BTree {
  ...
  uint64_t scan(Key k, int range, Value* output, Key* keysOut = nullptr) {
    int restartCount = 0;
  restart:
    if (restartCount++)
      yield(restartCount);
