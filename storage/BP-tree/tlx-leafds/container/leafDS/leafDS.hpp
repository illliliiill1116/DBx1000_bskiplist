...
public:
  ...
  // DBx1000-specific: overwrite the value of an already-present key in
  // place, without touching insert()/remove()'s log bookkeeping. insert()
  // asserts num_deletes_in_log == 0 partway through its log-flush path, so a
  // naive remove()+insert() combo to "update" a key can trip that assertion
  // once the delete log is non-empty. get_index()+update_val_at_index() is
  // the same lookup has() already performs internally, just without the
  // log/block mutation, so it's safe to call once has(e) is known true
  // under the caller's existing lock. Returns false if e's key isn't
  // present (caller should treat that as a fresh insert instead).
  bool update_value_if_exists(element_type e) {
    size_t idx = get_index(std::get<0>(e));
    if (idx == N) {
      return false;
    }
