...
namespace tlx_dbx1000_detail {
// DBx1000-specific: index_BPtree.cpp always stores the value as a pointer to
// an itemid_t (system/helper.h), and some DBx1000 indexes are not unique
// (e.g. TPCC's CUSTOMER_LAST_IDX, where several customers can share a last
// name). Plain tlx::btree_map insert() rejects duplicate keys outright,
// which would either abort (if the caller asserts success) or silently drop
// the previous row if the caller blindly overwrites. Instead, chain the new
// value onto the existing one via itemid_t::next while still holding the
// leaf's write lock, mirroring the equivalent hacks in
// storage/OLC_bskiplist/src/bsl_insert.c, storage/RW_bskiplist/src/insert.cpp,
// and the identical helper in tlx-plain/container/btree.hpp. Layout must
// match itemid_t exactly: {type, location, next, valid}.
        ...
        else // n->is_leafnode() == true
        {
            ...
            if (!allow_duplicates && leaf->slotdata.has(key)) {
                ...
                if constexpr (is_pair) {
                    // see tlx_dbx1000_detail::dbx1000_chain_onto above: chain
                    // the new value onto the existing one instead of
                    // rejecting the insert, while the leaf write lock (or,
                    // non-concurrent, exclusive ownership) is still held.
                    // Overwrite in place via LeafDS::update_value_if_exists()
                    // (added specifically for this) rather than remove() +
                    // insert(), which trips an internal LeafDS assertion
                    // once its delete log is non-empty.
                    uint64_t old_val = (uint64_t)leaf->slotdata.value(key);
                    tlx_dbx1000_detail::dbx1000_chain_onto(old_val, (uint64_t)value.second);
                    leaf->slotdata.update_value_if_exists(value);
                }
                if constexpr (concurrent) {
